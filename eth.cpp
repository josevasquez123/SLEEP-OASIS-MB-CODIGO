...
void Eth::reconnect(void){
  while (!client.connected()) {
    ...
    char *str[] = {"PUERTA"};                     //SE ESCRIBE TODO LOS TOPICOS MQTT QUE SE QUIERAN SUSCRIBIR
    ...
    if (client.connect(clientId.c_str(),"billeteronv11","billeterownr123")){
      ...
      client.subscribe("/SEDE2/MB/PUERTA/ALIVE/");
      client.subscribe("/SEDE2/MB/PUERTA/PUERTA/");

      for (int i=0; i<_nRooms; i++){                                                
        for (int j=0; j<1; j++){                                                    //J<# -> EL # DEBE SER EL TAMAÑO DEL ARRAY DE STR[]
          char buf[30];
          if (i<9){
