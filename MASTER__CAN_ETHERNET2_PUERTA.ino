...
void callback(char* topic, byte* payload, unsigned int length){
  ...
  if(topico == "/SEDE2/MB/PUERTA/ALIVE/"){
    ethWIZ.client.publish("/SEDE2/MB/PUERTA/ALIVE/ACK/", "true");
  }

  if(topico == "/SEDE2/MB/PUERTA/PUERTA/"){
...
void check_income_msg(void){
  if(!digitalRead(canMCP.intPin)){                                    
    ...
    if(rxBuf[0] == 0x09){
      ...
      ethWIZ.client.publish("/SEDE2/PUERTA/CABINA/", room_id_temp);
    }
